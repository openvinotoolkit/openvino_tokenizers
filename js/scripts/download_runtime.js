...
async function downloadRuntime(
  ...
  const runtimeArchiveUrl = getRuntimeArchiveUrl(packageJson, version, osInfo);
 ...
 * @returns {string}
 */
function getRuntimeArchiveUrl(packageJson, version, osInfo) {
  const {
    host,
  ...
  const fullPath = fullPathTemplate
    ...
    .replace(
      ...
      convertPlatformLabel(osInfo.platform),
    )
    .replace(new RegExp('{arch}', 'g'), convertArchLabel(osInfo.arch));

  return new URL(fullPath, host).toString();
}

function convertPlatformLabel(platform) {
  switch (platform) {
    case 'win32':
      return 'windows';
    case 'linux':
...
function convertArchLabel(arch) {
  switch (arch) {
    case 'x64':
      return 'x86_64';
    case 'arm64':
